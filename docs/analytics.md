...
Dashboard: `/admin` (HTTP Basic authentication, username `admin`). Always use HTTPS in production. Analytics is disabled until explicitly configured. Missing/short admin credentials fail closed. Dashboard and API responses are never publicly cached. No analytics records are included in the public client bundle.
...
On server startup, instrumentation creates only the analytics table and indexes, and does not modify existing application tables. For manual setup, run `node apps/web/lib/analytics/manage.mjs init` with the database variable. Deploy after variables are configured: the root layout includes the tracker at build time, and the Dockerfile accepts `ANALYTICS_ENABLED` as a build argument. A runtime toggle also stops ingestion immediately.
