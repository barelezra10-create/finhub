...
export async function dashboard(f:Filters) {
  ...
  const [summary,previous,daily,realtime,coverage,options,pages,sources,channels,engines,referrers,campaigns,countries,regions,cities,devices,browsers,systems,languages,keywords,outbound,offers,offerPages] = await Promise.all([
    ...
    grouped('path','',100),grouped('source'),grouped('channel'),grouped('search_engine',"AND search_engine<>''"),grouped("COALESCE(NULLIF(referrer,''),'Not supplied')"),grouped("source || ' / ' || medium || ' / ' || campaign", "AND campaign<>''"),grouped("COALESCE(NULLIF(country,''),'Unknown')"),grouped("CASE WHEN region='' THEN 'Unknown' ELSE region || ', ' || country END"),grouped("CASE WHEN city='' THEN 'Unknown' ELSE city || ', ' || country END"),grouped('device'),grouped('browser'),grouped('os'),grouped("COALESCE(NULLIF(language,''),'Unknown')"),grouped("keyword_type || ': ' || keyword","AND keyword<>''"),grouped('target',"AND kind='outbound'"),grouped("offer || ' / ' || COALESCE(NULLIF(placement,''),'Unspecified')","AND kind='outbound' AND offer<>''",100),
    pool.query(`SELECT path AS label,count(*) FILTER(WHERE kind='pageview')::int AS views,count(*) FILTER(WHERE kind='outbound' AND offer<>'')::int AS clicks,count(DISTINCT visitor) FILTER(WHERE kind='outbound' AND offer<>'')::int AS visitors FROM fintiex_analytics WHERE ${range} GROUP BY path HAVING count(*) FILTER(WHERE kind='outbound' AND offer<>'')>0 ORDER BY clicks DESC, views DESC LIMIT 100`,args),
  ]);
  return {summary:summary.rows[0],previous:previous.rows[0],daily:daily.rows,realtime:realtime.rows[0].visitors,coverage:coverage.rows[0],options:options.rows[0],pages,sources,channels,engines,referrers,campaigns,countries,regions,cities,devices,browsers,systems,languages,keywords,outbound,offers,offerPages:offerPages.rows as MetricRow[]};
}
export async function eventLog(f:Filters,page:number,kind:string) {
  ...
  const [events,count]=await Promise.all([
    db().query(`SELECT event_id,created_at,kind,path,source,medium,campaign,target,country,region,city,device,browser,os,language,referrer,channel,search_engine,keyword,keyword_type,offer,placement FROM fintiex_analytics WHERE ${where} ORDER BY created_at DESC,event_id DESC LIMIT 50 OFFSET $7`,[...args,(page-1)*50]),
