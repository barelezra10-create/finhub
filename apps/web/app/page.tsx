...
export default function Home() {
  ...
  return <div className="finance-home">
    ...
    <section className="hub-wrap hub-section savings-layout">
      ...
      <div className="savings-board"><div className="board-heading"><h3>Savings account snapshot</h3><span>Checked {SAVINGS_CHECKED}</span></div><div className="rate-labels"><span>Account & conditions</span><span>APY</span></div>{offers.map(o => <Link href={o.review} className="savings-row" key={o.key}><BrandLogo brand={o.key === "cit-savings-connect" ? "cit" : o.key} size={36}/><div><h4>{o.name}</h4><p>{o.condition}</p></div><strong>{rateLabel(o.apy)}</strong><ArrowUpRight size={17}/></Link>)}<p className="board-note">Dated observations, not live quotes. Variable rates and eligibility apply. This selection does not cover the whole market.</p></div>
    </section>
