...
export default function Page() {
  ...
  return <article>
    ...
    <section id="providers" className="max-w-(--max-w-page) mx-auto px-6 py-14">
      ...
      <p className="text-mute leading-relaxed max-w-3xl mb-7">These {providers.length} profiles are listed alphabetically. Each identifies the provider’s role, the facts checked and their sources. Product rules stay attached to the named policy; Prudential’s profile uses a dated brochure and does not confirm current sales availability.</p>
      <div className="grid md:grid-cols-2 gap-5">{providers.map(c => <div key={c.slug} className="card p-6"><h3 className="font-display font-bold text-xl mb-3">{c.carrier}</h3><p className="text-sm text-mute mb-3">{c.checked_profile?.role} · Sources checked {c.source_checked}</p><p className="text-mute mb-5">{c.checked_profile?.summary}</p><div className="flex flex-wrap gap-3"><Link href={`/insurance/life/${c.slug}`} className="u-link">Read profile →</Link><a href={c.quote_url} className="u-link" rel="nofollow noopener noreferrer" target="_blank">Provider website ↗</a></div></div>)}</div>
